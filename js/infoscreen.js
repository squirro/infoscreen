	...
	jQuery.getJSON(url).success(function(data) {
		...
		for (i = 0; i < len; i++) {
			...
			if (!fieldname.startsWith('#')) {
				...
				map[k_preload] = true;
				if (map[k_link].startsWith("http://www.youtube") || map[k_link].startsWith("http://youtube")) {
					console.info('No preload for: ' + map[k_link]);
					map[k_preload] = false;
				}
				content.push(map);
			}
